...
async def start(event):
  await event.reply("**🌀Uslanmaz Tagger Bot Aktif!!!**\n **Sahibim @serserizarog Fazla Yetkiye İhtiyaç Duymam Sadece Grubunuzda Boş Yetki Vermeniz Yetlir Olacaktir \nKomutlar için  /help Butonuna Tiklayiniz :)**",
                    buttons=(
...
async def mentionall(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Bu komutu gruplar ve kanallar için geçerli Dostum❗** \n ☄️")
  
  admins = []
  async for admin in client.iter_participants(event.chat_id, filter=ChannelParticipantsAdmins):
    admins.append(admin.id)
  if not event.sender_id in admins:
    return await event.respond("**Bu komutu sadace yoneticiler kullana bilir Admım〽️** \n ☄️")
  ...
  if event.pattern_match.group(1):
    ...
    return await event.respond("Etiket Yapmak için sebeb yok Dostum❗️")
  else:
    return await event.respond("**Etikete Başlamak için sebeb yaz Adamım...!**")
...
async def mentionall(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Bu komutu gruplar ve kanallar için geçerli Adamım❗️** \n☄️")
  
  admins = []
  async for admin in client.iter_participants(event.chat_id, filter=ChannelParticipantsAdmins):
    admins.append(admin.id)
  if not event.sender_id in admins:
    return await event.respond("**Bu komutu sadace yoneticiler kullana bilir Dostum〽️** \n ☄️")
  ...
  if event.pattern_match.group(1):
    ...
    return await event.respond("Başlatmak için sebeb yok Dostum❗️")
  else:
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      if event.chat_id not in anlik_calisan:
        await event.respond("Işlem Başarıyla Durduruldu\n\n**Buda bizim reklamımız ola bilir @UslanmazArsiv**❌")
        return
      if usrnum == 5:
        await client.send_message(event.chat_id, f"{usrtxt}\n\n{msg}")
        await asyncio.sleep(2)
...
async def handler(event):
    ...
    await event.reply('**Hey Bot Çalışıyor Merak Etme Sahibim Burdayım 🤓🤞** \n Sahibim @Serserizarog')
...
async def handler(event):
    ...
    await event.reply('__Botun Reklam Menüsü__\n**Reklam Veya İş Birliğimi Yapmak İstiyorsunuz Botun** [Sahibi](https://t.me/serserizarog) **İle İletişime Geçe Bilirsin😸**')
